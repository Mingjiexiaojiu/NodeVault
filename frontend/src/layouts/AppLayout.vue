      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <div class="hidden sm:flex items-center gap-0.5">
            ...
            <RouterLink v-if="auth.isSuperAdmin" to="/admin" class="px-3.5 py-1.5 text-sm font-medium rounded-lg transition-all duration-150 flex items-center gap-1"
              :class="String(route.path).startsWith('/admin') ? 'text-purple-700 bg-purple-50 font-semibold' : 'text-purple-500 hover:text-purple-900 hover:bg-purple-50'">
          ...
          <div class="relative flex items-center">
            ...
            <div v-if="showUserMenu" class="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-gray-100 py-1.5 z-50" style="box-shadow: 0 8px 32px rgba(0,0,0,0.10), 0 2px 8px rgba(0,0,0,0.06)" @mouseleave="showUserMenu = false">
              ...
              <RouterLink to="/credentials" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors" @click="showUserMenu = false">
