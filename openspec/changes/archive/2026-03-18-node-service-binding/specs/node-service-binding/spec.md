...
### Requirement: 按来源凭证过滤节点
系统 SHALL 支持在 `GET /api/v1/nodes` 添加 `?source_credential_id=<uuid>` 查询参数，仅返回来自该凭证（服务）的节点。

#### Scenario: 按 source_credential_id 过滤
- **WHEN** 携带 `?source_credential_id=<uuid>` 查询节点列表
- **THEN** 系统 SHALL 仅返回 `source_credential_id` 匹配的节点

#### Scenario: 过滤结果为空
- **WHEN** 指定的 credential_id 下没有任何导入节点
- **THEN** 系统 SHALL 返回空列表（200，items 为空数组）
...
- **THEN** 系统 SHALL 返回该凭证下所有节点的 `[{node_id, source_path, name}]` 列表
...
前端服务发现页面在探测完成后，如果当前选择了凭证，SHALL 请求已导入路径并对探测结果做 diff 标记：已导入（不可勾选，标绿）、新增（可勾选）、已删除（标灰警告）。
