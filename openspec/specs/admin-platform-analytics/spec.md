...
### Requirement: Top nodes ranking（更新）
系统 SHALL 提供 `GET /api/v1/admin/analytics/top-nodes` 端点，返回按调用次数排序的前 N 个节点（默认 10）。响应 SHALL 包含 organization_name 和 team_name 字段，替代原 namespace/slug 字段。
